...
///
/// The matching private seed never touches this repo or the R2 bucket. Tag CI
/// reads it from the `MEOWWATCH_RELEASE_KEY` GitHub Actions secret on the
/// `release` environment (see `tool/release_signer.dart`). The download's
/// SHA-256 lives in the same bucket as the zip, so anyone who can swap the zip
/// can swap the hash to match. A signature made with a key the bucket never
/// sees can't be forged that way, so it — not the hash — is the real trust
/// root.
///
