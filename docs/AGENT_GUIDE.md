...
- **Rotating is a two-release dance** (existing installs verify against the key *they* already carry, so the switch can't be abrupt):
  1. **Transitional release** — bake the *new* public key, but sign the zip with the **OLD** private key so current installs still accept it: `dart run tool/release_signer.dart sign --allow-key-mismatch <old-key> MeowWatch-windows-x64.zip MeowWatch-windows-x64.zip.sig`. Without `--allow-key-mismatch` the signer refuses (the seed's public half won't match the newly-baked key).
