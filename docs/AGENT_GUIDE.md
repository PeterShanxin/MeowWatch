...
Dependabot PRs, same-repo PRs from anyone other than **PeterShanxin**, and
...
author) always use GitHub-hosted `windows-2022`. Write access is not host trust.
Owner same-repo PRs **triggered by PeterShanxin** still default to self-hosted
so the owner's feedback pushes stay off the hosted-minutes meter
...
- **PR gate (analyze + test)** → **fork, Dependabot, collaborator PRs, and
  ...
  **push/tag**, take **`check-self-hosted`** on
  **`[self-hosted, windows, meowwatch-ci]`** unless the PR has **`ci-hosted`**.
  The suite is **not cross-platform** —
...
Registered to this repo with the **`self-hosted` + `windows` + `meowwatch-ci`** labels (those labels are what the workflow selects — not a machine hostname). It must be **online** for tag builds and for **PeterShanxin same-repo** `check-self-hosted` jobs **triggered by PeterShanxin** — otherwise `build-windows-x64` (tags) or that PR check queues until a runner appears. **Fork, Dependabot, collaborator PRs, and collaborator pushes onto an owner PR never use this runner**; they take hosted Windows automatically. For an owner PR you can sidestep the wait by adding the **`ci-hosted` label**; a tag build has no such fallback and always needs the runner. Do not invite outsiders to attach runners to this canonical repo.
