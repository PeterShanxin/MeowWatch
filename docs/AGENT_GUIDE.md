...
- **`build-windows-x64`** → GitHub-hosted **`windows-2022`**,
  ...
  and only when `github.actor` is PeterShanxin or ianmeowmeow** — pull
  requests cannot schedule it, and any other login's `v*` tag must not
  build or sign. Fork PRs never reach this job. The `Sign release` step
  writes `secrets.MEOWWATCH_RELEASE_KEY` (seed file contents) to
  `$RUNNER_TEMP`, runs `dart run tool/release_signer.dart`, then deletes
  the file. Missing or empty secret fails the job. Workflow default
  `permissions` are `contents: read`; this job is the one that gets
  `contents: write` to create GitHub Releases.
- **`release`** (R2 upload) → `ubuntu-latest`. Bash/rclone job, tag-push
  only, after a successful signed zip + GitHub Release. If GitHub-hosted
  Actions is temporarily unavailable, both the Windows build and R2 stay
  stale. Re-run the workflow after service recovery (no re-tag needed),
  or use the documented signed manual publish procedure. `secrets.R2_*`
...
- **Keys.** Tag CI reads the private **seed** from the GitHub Actions secret **`MEOWWATCH_RELEASE_KEY`** on the **`release`** environment — the same base64 seed-file contents `tool/release_signer.dart` expects, not a path on the runner. Never commit it. Never put it in R2. Never interpolate it into PR / analyze / test jobs. A local seed file is valid for manual signing (`MEOWWATCH_RELEASE_KEY` as a file path, or `%USERPROFILE%\.meowwatch\release-key.txt`). The matching **public** key is baked into the app as `releasePublicKeyBase64` in `lib/core/update/release_signature.dart`. Generated 2026-07-06.
- **Signing is automatic on tag builds.** On a trusted-admin `v*` tag, the hosted Windows `Sign release` step writes the secret to `$RUNNER_TEMP`, runs `dart run tool/release_signer.dart sign …`, then deletes the file, producing `MeowWatch-windows-x64.zip.sig`. The R2 `release` job embeds it into `latest.json` as the asset's `sig`. **Tag-push only** — no PR or fork can invoke the signing step. The signer refuses to sign if the seed's public half doesn't match the baked-in key (guards against signing a release the app would reject).
...
- `core/nearby/` — an opt-in mobile companion for the active session. `DesktopNearbyService` owns the Private-interface listener, protected identity, pairing approval and shutdown; `DesktopCompanionSession` adapts commands to the existing player and Syncplay client. The shared protocol/storage packages are pinned to a public MeowWatch-Mobile commit. `GuardedVideoControls` rechecks session, control lease and media generation inside the native-player lock immediately before dispatch. Do not replace this with a check made before an asynchronous player wait. See `docs/NEARBY_COMPANION.md` for setup and acceptance boundaries.
