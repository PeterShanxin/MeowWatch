...
**Glue:** `PlaybackSyncBridge` wires `VideoCore` ⇄ `SyncCore` (local playback → outgoing State; incoming peer state → local seek/pause via `decideFollow`). `HomeScreen` owns a `SessionServices` host keyed by `SessionMode` (`local` / `synced`): a synced session constructs `SyncplayClient` + `PlaybackSyncBridge` + `ChatStore` and connects as before; a local session skips that trio and mounts the same player surface without multiplayer chrome. `SessionChrome.forMode` is the one UI gate; `selectSessionBanner` keeps media/load transients in local while dropping derived sync banners. The lobby toggle (`kLocalPlayerModeSettingKey`) only changes the default Start / Continue Watching path — join-code and saved rooms stay synced. Those two actions await the first settings load so a persisted ON cannot race the default-off seed. An explicit lobby toggle bumps a revision so a stale in-flight read cannot overwrite the newer choice.
