...
- **Start/stop it yourself, on demand — don't ask the user.** The runner is **not** auto-started — no service, no logon autostart (user's explicit choice; don't add one). The agent driving a release **starts it before pushing a `v*` tag and stops it once the release run is green and R2 is verified** — leaving it running idle is not the intent. Confirm it's live with `gh api repos/PeterShanxin/MeowWatch/actions/runners` (`status: online`) or `Get-Process Runner.Listener`. If a PR's `check-self-hosted` (Analyze & Test (self-hosted)), a tag's `check-self-hosted`, **or** `build-windows-x64` sits **queued**, the runner is offline.
  - **GitHub deletes the registration if the runner stays offline for ~2 weeks, and only the runner's own log says so.** `gh api repos/PeterShanxin/MeowWatch/actions/runners` then reports `total_count: 0` while `C:\actions-runner\.runner` still looks perfectly configured, and `Start-Process run.cmd` exits immediately with nothing on screen — `_diag\Runner_*.log` is where you find `Registration was not found` / `The runner registration has been deleted from the server, please re-configure`. Happened 2026-08-18, last connection 2026-07-31. **Recovery:** `config.cmd remove --local` (the server-side registration is already gone, so the normal token remove has nothing to talk to), then `config.cmd --unattended --url https://github.com/PeterShanxin/MeowWatch --token <registration-token> --name meowwatch-pc --work _work`, minting the token with `gh api -X POST repos/PeterShanxin/MeowWatch/actions/runners/registration-token --jq .token` and never writing it to a file. Labels come back as the implicit `self-hosted, Windows, X64`, which is what `runs-on: [self-hosted, windows]` matches. **Prevention:** this is a consequence of the on-demand model, so expect it after any quiet fortnight and check `total_count` before concluding a queued job means "just start the runner".
  - **Refresh the action archive cache first** (see the next bullet), because the runner reads `.env` once at listener start: `dart run tool/action_cache.dart sync`.
