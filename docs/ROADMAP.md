...
| 6 🚧 | Extras | **In progress (built one-at-a-time on `main`, not yet tagged).** Shipped: auto-pause on sync drop/friend-leave (2s debounce); friend-joined banner; room code in player gear (copyable + snackbar) and connect-screen "copied" toast; **file-mismatch warning** (peer file surfaced from Set/List, byte-size-first compare); **idle cat mascot** (breathes + blinks on empty screen); **chat control channel** (sentinel-prefixed reactions/typing piggybacked on chat); **floating reactions** (emoji burst over video, with an emoji palette button); **typing indicator** ("X is typing…"). Already-done earlier: watch history (P4), presence + HH:MM timestamps (P3). **Deferred** (see backlog): PiP, per-message reactions. | _TBD_ |
